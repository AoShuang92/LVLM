...
def main():
    ...
    dataset_train = [format_data(sample, prompt) for sample in ds['validation']]
    dataset_val = [format_data(sample, prompt) for sample in ds['test']]
    
    # ds_train = load_dataset("merve/vqav2-small",split="validation[:30%]")
    # ds_val = load_dataset("merve/vqav2-small",split="validation[:30%]")
    ...
    args = SFTConfig(
        output_dir="fine-tuned-visionllama-pvqa", # directory to save and repository id
